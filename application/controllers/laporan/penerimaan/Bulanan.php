...
class Bulanan extends CI_Controller {
    ...
    function index() {
        ...
        $this->load->view("laporan/penerimaan/v_bulanan", $data);
    }
    ...
    function print_laporan() {
        if (isset($_GET["tgl1"]) && isset($_GET["tgl2"]) && isset($_GET["idb"])) {
            ...
            $data = $this->pbs->lhb_get(3, $tgl1, $tgl2, $id_bank);
            
            require_once(APPPATH. "libraries/tcpdf/pdf.php");
            $pdf = new PDF(TRUE, TRUE, "P", "A3");
            
            $pdf->SetFont("cid0cs", "B", 12);
            $pdf->cell($pdf->GetPageWidth(), $pdf->get_cell_height() + 2.5, "Laporan Donatur Bulanan", 0, 1, "C");
            ...
            for ($i = $tgl1; $i <= $tgl2; $i = date("Y-m", strtotime($i. " + 1 month"))) {
                $row = $data->row($j);
                $bulan = get_month_name(explode("-", $i)[1]). " " .explode("-", $i)[0];
                ...
                $pdf->cell($t1_cell["width"][0], $t1_cell["height"][1], $bulan, 1, 0, "C");
            ...
            $pdf->Output("LaporanBulanan_" .$tgl1.($tgl1 === $tgl2 ? "" : "_" .$tgl2). ".pdf", "I");
        } else {
